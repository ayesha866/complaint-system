   ...
   const handleSubmit = async (e,user_id) => {
    e.preventDefault();
       if (!updateAgent.name && !updateAgent.email && !updateAgent.phone) {
         ...
         axios.put(`http://localhost:8000/api/users/${user_id}`, updateAgent, {
   headers: {
...
const token = user?.token;// adjust key if needed
                           ...
                           return (
                              <tr key={agent._id}>
                                 ...
                                 <td><Button onClick={() => handleToggle(agent._id)}
                                    ...
                                    <Collapse in={open}>
                                       <Form onSubmit={(e) => handleSubmit(e,agent._id)} className='p-5'>
